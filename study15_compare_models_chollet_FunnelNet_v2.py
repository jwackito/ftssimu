...
def build_Chollet_Jena_10ch_model(shp):
    ...
    x = layers.Conv1D(neurons, 5, padding='valid')(x)
    x = layers.LeakyReLU()(x)
    x = layers.LSTM(neurons, dropout=.1, recurrent_dropout=.50)(x)
    tsoutput = layers.Dense(1)(x)
...
def transform_data(tss):
    ...
    channels = len(keys)
    for k in keys:
        trainx.append((log1p(tss[k]) - tssmu[k])/tssstd[k])
    trainx = np.array(trainx)
    trainy = (log1p(tss['target']) - tssmu['target'])/tssstd['target']
    trainx = numpy.reshape(trainx, (trainx.shape[1], trainx.shape[2], channels))
    trainpwx = [(log1p(tss['target_bytes']) - tssmu['target_bytes'])/tssstd['target_bytes'], (log1p(tss['target_nxfers']) - tssmu['target_nxfers'])/tssstd['target_nxfers']]
...
def trainvalChollet(tss, tss_val):
    ...
    loss = model.fit({'ts_input':traintsx},
            {'moutput': trainy}, epochs=chollet_epochs, batch_size=1024, shuffle=False, 
...
def predictWithChollet(tss, model):
    ...
    tss['pchollet'] = np.e**((preds*tssstd['target'])+tssmu['target'])-1
    return tss
...
def trainvalFunnel(tss, tss_val):
    ...
    loss = model.fit({'ts_input':traintsx, 'pw_input':trainpwx, 'emb_input':trainembx},
            {'moutput': trainy}, epochs=funnel_epochs, batch_size=1024, shuffle=False, 
...
def predictWithFunnel(tss, model):
    ...
    tss['pfunnel'] = np.e**((preds*tssstd['target'])+tssmu['target'])-1
    return tss
...
def predict_and_fogp_chollet(tss, thr=.1):
    targets = []
    pmodel = []
    tss = predictWithChollet(tss,chollet_model)
    targets = np.array(tss['target'])
    pchollet = np.array(tss['pchollet'])
    return targets, pchollet, fogp(targets, pchollet.flatten(), thr)

def predict_and_fogp_funnel(tss, thr=.1):
    targets = []
    pmodel = []
    tss = predictWithFunnel(tss, funnel_model)
    targets = np.array(tss['target'])
    pfunnel = np.array(tss['pfunnel'])
    return targets, pfunnel, fogp(targets, pfunnel.flatten(), thr)
...
#train_val = readtss('validation_7jul-8jul')
...
for k in ['bytes', 'fbytes', 'ubytes', 'nxfers', 'fnxfers', 'unxfers', 'minttc', 'medianttc', 'meanttc', 'maxttc', 'target_bytes', 'target_nxfers', 'target']:
    tssmu[k] = log1p(train[k]).mean()
    tssstd[k] = log1p(train[k]).std()
...
for neurons in [8,16,32,64,128]: 
    callbackslist = [ 
            ...
            keras.callbacks.ModelCheckpoint(filepath='/tmp/Funnel-{epoch:02d}-{val_loss:.2f}.hdf5',monitor='val_loss', save_best_only=True, save_weights_only=True)] 
    train, loss_funnel, funnel_model = trainvalFunnel(train, train_val) 
    histories.append(loss_funnel) 
    targets, pfunnel, fogpfunnel = predict_and_fogp_funnel(test, thr=.1) 
    fogps.append(fogpfunnel)
...
for neurons in [8,16,32,64,128]: 
    callbackslist = [ 
            ...
            keras.callbacks.ModelCheckpoint(filepath='/tmp/Chollet-{epoch:02d}-{val_loss:.2f}.hdf5',monitor='val_loss', save_best_only=True, save_weights_only=True)] 
    train, loss_chollet, chollet_model = trainvalChollet(train, train_val) 
    histories.append(loss_chollet) 
    targets, pchollet, fogpchollet = predict_and_fogp_chollet(test, thr=.1) 
    fogps.append(fogpchollet)
...
callbackslist = [
            keras.callbacks.EarlyStopping(monitor='val_loss', patience=3, restore_best_weights=True),
            keras.callbacks.ModelCheckpoint(filepath='/tmp/FunnelNet-{epoch:02d}-{val_loss:.2f}.hdf5',monitor='val_loss', save_best_only=True, save_weights_only=True)]
train, loss_funnel, funnel_model = trainvalFunnel(train, train_val)

chollet_model.save('CholletModel_60epochs_recurrentdropout.model')
funnel_model.save('FunnelModel_60epochs_recurrentdropout.model')


#chollet_model = tf.keras.models.load_model('CholletModel_60epochs_recurrentdropout.model')
#funnel_model = tf.keras.models.load_model('FunnelModel_60epochs_recurrentdropout.model')
...
plt.title('MAE Loss Train/Validation FunnelNet Model')
plt.legend()
