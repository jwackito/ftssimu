...
xfers = read_xfers('data/transfers-FTSBNL-20190606-20190731.csv', nrows=8796970)

rules = pd.read_csv('data/xfers_per_rule.csv', nrows=10)
...
#model_name = 'data/polymodels3/model_polydeg1_lss_trainedwith_link-agnostic_89832__linkqueued_max-linksactive_max-linkmeanthr-retry_count-SIZE-ftsqueued_mean-ftsqueued_median-ftsbqueued_median-ftsqueued_95p__.model'
model_name = 'data/polymodels3/model_polydeg1_lss_trainedwith_link-agnostic_logpred_86167__linkqueued_median-linkbqueued_median-linkbqueued_95p-linksactive_max-linkmeanthr-retry_count-SIZE-ftsbqueued_median-ftsqueued_95p__.model'
...
def get_prediction_for_rule(r):
    ...
    for t in r.itertuples():
        if str(t.link) is not 'nan':
            ...
            dirty = 1
            continue
    X = pd.DataFrame(r, columns=model_columns).values
    prediction = model.predict(X)
    pred_mean = prediction.mean()
    pred_median = np.median(prediction)
    pred_95p = np.percentile(prediction,95)
...
for cname in df2.columns[1:]:
    try:
        ...
        continue
df2.to_csv('data/rule_ttc_predction_v2_logpred.csv')
