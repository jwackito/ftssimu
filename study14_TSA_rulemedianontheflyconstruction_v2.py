...
def get_prediction_for_rule(rid, rho, lags, lookback):
    ...
    tsobs.to_csv('data/timeseries/obs_%s.csv.bz2'%r.ruleid, index=False, compression='bz2')
    tsreal.to_csv('data/timeseries/real_%s.csv.bz2'%r.ruleid, index=False, compression='bz2')
...
for run in range(30):
    ...
    for rho in rhos:
        ...
        rids.to_csv('data/tsa/TSA_predictions_june_rho_%d_run_%d.csv'%(rho,run), index=False)
...
for run in range(30):
    ...
    for lag in lags:
        for lb in lookback:
            ...
            rids = pd.DataFrame(rids, columns=['rid', 'ttc', 'obspmin', 'obspmedian', 'obspmean', 'obspmax', 'realpmin', 'realpmedian', 'realpmean', 'realpmax'])
            rids.to_csv('data/tsa/TSA_predictions_rho_%d_lags_%d_lookback_%d_run_%d.csv'%(rho, lag, lb, run), index=False)
