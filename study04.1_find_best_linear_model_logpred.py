...
for degree in [1]:
    for algo in ['lss']:
        ...
        for i in range(1):
            ...
            ytr = np.log(df_train.RTIME + df_train.QTIME + df_train.NTIME)
            ...
            yte = np.log(df_test.RTIME + df_test.QTIME + df_test.NTIME)
            stimes_test = df_test.stimes

            for ncomponents in range(1,10):
                for idx in combinations(list(observables.values()), ncomponents):
                    model_name = 'data/polymodels3/model_polydeg%d_%s_trainedwith_%s_logpred_%04d__'%(degree,algo,linkname,modelindex)+'-'.join(idx)+'__.model'
                    modelindex += 1
