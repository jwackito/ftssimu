...
ftsstatecolumns = pd.read_csv('data/ftsstate_stats_all_2019.csv', nrows=1).columns
ftsstate = pd.read_csv('data/ftsstate_stats_all_2019.csv', nrows=1000000, skiprows=7000000, names=ftsstatecolumns)
...
np.random.seed(42)
# sample 50K
mysample = xfers[(xfers.link.astype(str) != 'nan') & (xfers.state == 'D') & (xfers.ended < ftsstate.event_time.max()) & (xfers.created > ftsstate.event_time.min())]
...
mysample.to_csv('data/trainpolydata_1Million_xfers_2019.csv', index=False)
