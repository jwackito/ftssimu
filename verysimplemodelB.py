##################
# MODEL 10 (Using the real rate per transfer)
##################
...
while current_time < xfers.loc[len(xfers)-1].submitted or current_active > 0 or len(queued_transfers) > 0:
    ...
    for xfer in active_transfers:
        active_transfers[xfer]['remaining'] -= active_transfers[xfer]['real_rate']
...
modelB_active = n_active
modelB_queued = n_queued
modelB_qoutput = queue_output
modelB_noutput = network_output
modelB_ended_transfers = ended_transfers
