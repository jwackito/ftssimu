...
def get_real_finished_rules_at(ts, window=60):
    cut = rules[(rules.min_created >= (ts - dt.timedelta(seconds=window))) & (rules.min_created < ts)]
    return cut
...
def generate_median_mean_pred_for_rule(rid, periods=60):
    ...
    cut = get_real_finished_rules_at(r.min_created, periods)
...
for run in range(100):
    print('############### RUN  %i ################'%run)
    indices = np.random.choice(rules.index, 100)
...
pd.DataFrame(fogp_wmin).to_csv('data/fogp_rules_stats_for_real_min.csv',index=False)
pd.DataFrame(fogp_wmedian).to_csv('data/fogp_rules_stats_for_real_median.csv',index=False)
pd.DataFrame(fogp_wmean).to_csv('data/fogp_rules_stats_for_real_mean.csv',index=False)
pd.DataFrame(fogp_wmax).to_csv('data/fogp_rules_stats_for_real_max.csv',index=False)
