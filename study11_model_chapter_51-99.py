...
for run_number in range(100):
    print(run_number)
    pred_rules = Parallel(n_jobs=12, backend='multiprocessing')(delayed(get_prediction_for_rule)(r, run_number) for r in list(set(rules[(rules.nxfers >= 51)&(rules.nxfers < 100)&(rules.createdatonce == 1)].sample(500).ruleid)))
...
df.to_csv('data/rule_ttc_prediction_study11_51-99.csv', index=False)
