...
while current_time < xfers.loc[20000-1].submited or current_active > 0:
    ...
    for xfer in active_transfers:
        active_transfers[xfer]['remaining'] -= current_bw/max(1,current_active*.60)
...
model8_active = n_active
model8_queued = n_queued
model8_qoutput = queue_output
model8_noutput = network_output

plt.plot(model8_queued,label='model 8 queued')
#plt.plot(real_queued, label='real queued')
#plt.plot(model8_active, label='model 8 active')
