...
def read_xfers(path, src, dst):
    xfers = pd.read_csv(path, usecols=['account', 'activity', 'id', 'src_name', 'dst_name', 'source_rse_id','dest_rse_id', 'external_host', 'created_at', 'started_at', 'submitted_at', 'transferred_at', 'bytes'])
    ...
    print('Ignoring %d anomalous transfers'%(xfers_len - len(xfers)))
