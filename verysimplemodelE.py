##################
# MODEL 13 (Using rate prediction from a function)
#           Using smart actives
##################
...
max_actives2 = np.array([x for x in map(np.ceil, pd.rolling_window(max_actives, window=10, win_type='blackmanharris'))])
#max_actives2 = np.array([x for x in map(ceil, pd.rolling_window(max_actives, window=5, win_type='boxcar'))])
current_active = 0
current_queued = 0
...
while current_time < xfers.loc[len(xfers)-1].submitted or current_active > 0 or len(queued_transfers) > 0:
    ...
    unqueue = []
    ##################
    # Smart ACTIVE selection
    ##################
    try:
        current_maxactive = max(max_actives[int((current_time - offset_time).total_seconds())], max_actives2[int((current_time - offset_time).total_seconds())])
        max_active = current_maxactive
        used_actives.append(max_active)
        real_actives.append(max_actives[int((current_time - offset_time).total_seconds())])
    except IndexError:
        pass
...
modelE_active = n_active
modelE_queued = n_queued
modelE_qoutput = queue_output
modelE_noutput = network_output
modelE_ended_transfers = ended_transfers
