...
def get_prediction_for_rule(r):
    ...
    pred_using_real_thr = r.SIZE.sum()/rthrput
    dirty = 0
    for l in list(set(r.link)):
        if l is not np.nan:
            ...
            if link_multiplicity == 1:
                return [rid, created_at_once, dirty, link_multiplicity, ttc, rtime, qtime, ntime, submitted_std, started_std, rthrput, rsize, 0, 0, 0, 0, 0, 0, 0, 0]
            dirty = 1
            continue
    ...
    return [rid, created_at_once, dirty, link_multiplicity, ttc, rtime, qtime, ntime, submitted_std, started_std, rthrput, rsize, link_mean_thr, link_median_thr, link_95p_thr, link_max_thr, pred_using_real_thr, pred_using_mean_thr, pred_using_median_thr, pred_using_95p_thr, pred_using_max_thr] 
...
df = pd.DataFrame(np.array(pred_rules), columns=['rid', 'created_at_once', 'dirty', 'link_multiplicity', 'ttc', 'rtime', 'qtime', 'ntime', 'submitted_std', 'started_std', 'rthrput', 'rsize', 'link_mean_thr', 'link_median_thr', 'link_95p_thr', 'link_max_thr', 'pred_real', 'pred_mean', 'pred_median', 'pred_95p', 'pred_max'])
for cname in df.columns[1:]:
    df[cname] = df[cname].astype(float)
df.to_csv('data/rule_ttc_predction_study03b.csv', index=False)
